...
export default function NouveauDevisPage() {
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    const res = await fetch('/api/business/devis', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ client, lines, notes, valid_until: validUntil || null }),
    })
    if (!res.ok) { setErr((await res.json()).error ?? 'Erreur'); setLoading(false); return }
    router.push('/business/devis')
  }, [client, lines, notes, validUntil, router])

  return (
    <div style={{ padding: '28px 32px', maxWidth: 780 }}>
      <div style={{ marginBottom: 24 }}>
        <p style={{ fontSize: 12, color: '#64748B', textTransform: 'uppercase', letterSpacing: '.08em', marginBottom: 4 }}>Devis</p>
        <h1 style={{ fontSize: 22, fontWeight: 800, color: '#0F172A', margin: 0 }}>Nouveau devis</h1>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #E2E8F0', padding: 20, marginBottom: 20 }}>
          ...
          <div>
            ...
            <textarea style={{ ...inp, resize: 'vertical', minHeight: 60 }} value={notes} onChange={e => setNotes(e.target.value)} placeholder="Conditions de paiement, livraison…" />
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
          <button type="button" onClick={() => router.back()} style={{ padding: '10px 20px', borderRadius: 8, border: '1px solid #E2E8F0', background: '#fff', fontSize: 13, cursor: 'pointer', color: '#374151' }}>Annuler</button>
