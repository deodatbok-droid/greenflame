/**
 * POST /api/vouchers/emit
 *
 * Émet un bon d'achat depuis le pool mensuel de l'utilisateur.
 * Le bon est émis à la demande, pour un montant précis.
 * Valable chez tout marchand GreenFlame jusqu'à la fin du mois courant.
 *
...
export async function POST(req: NextRequest) {
  ...
  // Vérifier le pool disponible (lecture atomique via select for update)
  ...
  // Calculer l'expiration : dernier instant du mois courant
  const now  = new Date()
  const year = now.getFullYear()
  const month = now.getMonth() + 1  // 0-indexed → 1-indexed
  // Premier jour du mois suivant = dernier instant du mois courant
  const expiresAt = new Date(Date.UTC(year, month, 0, 23, 59, 59, 999)).toISOString()

  // Incrémenter emitted_fcfa de façon atomique
  const { error: updateErr } = await svc
    ...
    .eq('id', rights.id)
    .eq('emitted_fcfa', rights.emitted_fcfa)  // garde optimiste
